...
EventDrivenMap::EventDrivenMap(const arma::vec* pParameters, unsigned int noReal)
{
  ...
  // Read in parameters
  arma::fvec pfParameters = arma::fvec((*pParameters).n_elem);
  pfParameters = arma::conv_to<arma::fvec>::from(*pParameters);
  ...
  cudaMalloc( &mpDev_p, pfParameters.n_elem*sizeof(float) );
  ...
  cudaMemcpy(mpDev_p,pfParameters.begin(),pfParameters.n_elem*sizeof(float),cudaMemcpyHostToDevice);
...
void EventDrivenMap::ComputeF(const arma::vec& Z, arma::vec& f)
{
  ...
  printf("Beta = %f\n",par_val);
  EvolveKernel<<<mNoReal,mNoThreads>>>(mpDev_v,mpDev_s,mpDev_p,mpDev_w,mFinalTime,mpDev_firingInd,mpDev_firingTime,
      mpDev_spikeInd,mpDev_spikeCount,mpDev_lastSpikeInd,mNoReal,mNoStoredSpikes);

  cudaMemcpy(mpHostData,mpDev_firingTime,mNoReal*mNoStoredSpikes*sizeof(float),cudaMemcpyDeviceToHost);
  char filename[] = "testFiringTime.dat";
  SaveData(mNoReal*mNoStoredSpikes,mpHostData,filename);

  unsigned short* mpHost_firingInd;
  unsigned short* mpHost_spikeInd;
  unsigned short* mpHost_spikeCount;
  float* mpHost_firingIndf;
  float* mpHost_firingTime;
  float* mpHost_spikeIndf;
  float* mpHost_spikeCountf;

  mpHost_firingInd = (unsigned short*) malloc( mNoReal*mNoStoredSpikes*sizeof(short));
  mpHost_firingIndf = (float*) malloc( mNoReal*mNoStoredSpikes*sizeof(float));

  mpHost_firingTime = (float*) malloc( mNoReal*mNoStoredSpikes*sizeof(float));

  mpHost_spikeInd = (unsigned short*) malloc( mNoReal*mNoStoredSpikes*sizeof(short));
  mpHost_spikeIndf = (float*) malloc( mNoReal*mNoStoredSpikes*sizeof(float));

  mpHost_spikeCount = (unsigned short*) malloc( mNoReal*sizeof(short));
  mpHost_spikeCountf = (float*) malloc( mNoReal*sizeof(float));

  cudaMemcpy(mpHost_firingInd,mpDev_firingInd,mNoReal*mNoStoredSpikes*sizeof(short),cudaMemcpyDeviceToHost);
  cudaMemcpy(mpHost_firingTime,mpDev_firingTime,mNoReal*mNoStoredSpikes*sizeof(float),cudaMemcpyDeviceToHost);
  cudaMemcpy(mpHost_spikeInd,mpDev_spikeInd,mNoReal*mNoStoredSpikes*sizeof(short),cudaMemcpyDeviceToHost);
  cudaMemcpy(mpHost_spikeCount,mpDev_spikeCount,mNoReal*sizeof(short),cudaMemcpyDeviceToHost);

  for (int j=0;j<mNoReal*mNoStoredSpikes;++j)
  {
    mpHost_firingIndf[j] = (float)mpHost_firingInd[j];
    mpHost_spikeIndf[j]  = (float)mpHost_spikeInd[j];
  }
  for (int j=0;j<mNoReal;++j)
  {
    mpHost_spikeCountf[j] = (float)mpHost_spikeCount[j];
  }
  char filename2[] = "testFiringInd.dat";
  char filename4[] = "testSpikeInd.dat";
  char filename5[] = "testSpikeCount.dat";
  char filename6[] = "testFiringTime.dat";
  SaveData(mNoReal*mNoStoredSpikes,mpHost_firingIndf,filename2);
  //SaveData(mNoReal*mNoStoredSpikes,mpHost_firingTime,filename6);
  SaveData(mNoReal*mNoStoredSpikes,mpHost_spikeIndf,filename4);
  SaveData(mNoReal,mpHost_spikeCountf,filename5);
...
__global__ void Restrict( float *U, const float *averages,
    const unsigned int noReal)
{
  float xBar[noSpikes], tBar[noSpikes];
  float noCross;
  float tBarSq, xtBar;
  unsigned int i;
  ...
  if (index<noReal) {
    for (i=0;i<noSpikes;i++) {
      noCross    = averages[4*noSpikes*noReal+i*noReal+threadIdx.x];
      xBar[i]    = averages[0*noSpikes*noReal+i*noReal+threadIdx.x]/noCross; // Divide by noCross to correct scalings for offset
      tBar[i]    = averages[1*noSpikes*noReal+i*noReal+threadIdx.x]/noCross; // Divide by noCross to correct scalings for offset
      tBarSq     = averages[2*noSpikes*noReal+i*noReal+threadIdx.x];
      xtBar      = averages[3*noSpikes*noReal+i*noReal+threadIdx.x];
      speedNum   += xtBar-noCross*xBar[i]*tBar[i];  // Multiply by noCross to undo scalings from earlier
      speedDenom += tBarSq-noCross*tBar[i]*tBar[i]; // Multiply by noCross to undo scalings from earlier
    }
    speed = speedNum/speedDenom;
    U[index] = speed*(2.0f*L/noThreads);

    baseOffset = xBar[0]-speed*tBar[0];

    # pragma unroll
    for (i=1;i<noSpikes;i++) {
      U[index+i*noReal] = (baseOffset-xBar[i])/speed+tBar[i];
    }
  }
}
